...
mut_corr_pca2 <- map(ordered_muts_matrix, function(column){
  cor.test(pca_2, column) %>%
    tidy()
}) %>%
  bind_rows() %>%
  bind_cols(gene_name = colnames(ordered_muts_matrix)) %>%
...
sig_mut_corr_pca2 <- mut_corr_pca2 %>%
  mutate(p.adj = p.adjust(p.value, method = "fdr")) %>%
  dplyr::select(gene_name, estimate, p.value, p.adj, everything()) %>%
  print()
...
Pathway analysis of mutations
```{r}
library(gprofiler2)

# select all mutations significant at p < 0.05
# get gene names
selected_pca1_muts <- sig_mut_corr_pca1 %>%
  dplyr::filter(p.value < 0.05) %>%
  dplyr::filter(estimate > 0) %>%
  pull(gene_name) %>%
  print()

# test for enrichment with gprofiler
gostres_pca1 <- gost(query = selected_pca1_muts, 
                organism = "hsapiens", 
                ordered_query = FALSE, 
                multi_query = FALSE, 
                significant = TRUE, 
...
selected_pca1_muts_neg <- sig_mut_corr_pca1 %>%
  dplyr::filter(p.value < 0.05) %>%
  dplyr::filter(estimate < 0) %>%
  pull(gene_name) %>%
  print()
...
# test for enrichment with gprofiler
gostres_pca1_neg <- gost(query = selected_pca1_muts_neg, 
                organism = "hsapiens", 
                ordered_query = FALSE, 
                multi_query = FALSE, 
                significant = TRUE, 
...
selected_pca2_muts <- sig_mut_corr_pca2 %>%
  dplyr::filter(p.value < 0.05) %>%
  dplyr::filter(estimate > 0) %>%
  pull(gene_name) %>%
  print()

gostres_pca2 <- gost(query = selected_pca2_muts, 
                organism = "hsapiens", 
                ordered_query = FALSE, 
                multi_query = FALSE, 
                significant = TRUE, 
...
selected_pca2_muts_neg <- sig_mut_corr_pca2 %>%
  dplyr::filter(p.value < 0.05) %>%
  dplyr::filter(estimate < 0) %>%
  pull(gene_name) %>%
  print()

gostres_pca2_neg <- gost(query = selected_pca2_muts_neg, 
                organism = "hsapiens", 
                ordered_query = FALSE, 
                multi_query = FALSE, 
                significant = TRUE, 
