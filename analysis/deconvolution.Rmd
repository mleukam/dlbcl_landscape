...
Deriving a novel signature matrix. LM22 - the main micro-array-derived deconvolution signature matrix has some issues that render it less usable for DLBCL. 
...
keepers <- c("CD8+ T-cells", "Th1 cells", "Th2 cells", "Tregs", "Macrophages", "Macrophages M1", "Macrophages M2", "DC", "aDC", "Eosinophils")
...
keepers3 <- c("CD8+ T-cells", "Th1 cells", "Macrophages")

xc_tidy_filt3 <- xc_tidy_filt %>%
  dplyr::filter(cell_type %in% keepers3) %>%
  mutate(cell_type = fct_drop(cell_type)) %>%
  mutate(clust = fct_recode(clust, "Cold" = "Cold  (n = 361)",
                            "Intermediate-T" = "Intermediate-T  (n = 283)",
                            "Intermediate-M" = "Intermediate-M  (n = 225)",
  ...
  mutate(cell_type = fct_recode(cell_type, 
                                "CD8+ T Cells" = "CD8+ T-cells",
                                "CD4+ Th1 Cells" = "Th1 cells",
                                "Macrophages" = "Macrophages"))

# boxplot
ash_boxplot <- ggplot(xc_tidy_filt3, 
                        aes(x = clust, y = xc_score)) +
  geom_boxplot(aes(fill = clust)) +
  facet_grid(cols = vars(cell_type)) +
  ylab("xCell scores") +
  xlab("") +
  ggtitle("Immune Cell Deconvolution of DLBCL Transcriptomes") +
  theme_classic2() +
  scale_fill_d3() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  labs(fill = "Cluster", subtitle = "n = 1189") +
  stat_compare_means(aes(label = paste0("p = ", ..p.format..)),
                     method = "anova") +
...
ash_boxplot
# 5.5 wide x 4.5 tall
ggsave("output/ASH_ORAL_figures/deconvolution.png", ash_boxplot, height = 4.5, width = 5.5, units = "in")
```

Individual plots

```{r}
cd8_table <- xc_tidy_filt %>%
...
adc_table <- xc_tidy_filt %>%
  dplyr::filter(cell_type == "aDC") %>%
  mutate(cell_type = fct_drop(cell_type)) %>%
  mutate(clust = fct_recode(clust, "Cold" = "Cold  (n = 361)",
                            "Intermediate-T" = "Intermediate-T  (n = 283)",
                            "Intermediate-M" = "Intermediate-M  (n = 225)",
...
macrophage_plot <- ggplot(macrophage_table, 
                        aes(x = clust, y = xc_score)) +
  geom_boxplot(aes(fill = clust)) +
   ylab("") +
  xlab("") +
...
ggarrange(cd8_plot, th1_plot, m1_plot, m2_plot, adc_plot, nrow = 1)

ggsave("output/ASH_ORAL_figures/decon.combined.png", height = 3.8, width = 8.2, units = "in")
```
