...
#  .. .. ..$ gene_biotype   : chr [1:16178] "protein_coding" "protein_coding" "protein_coding" #"protein_coding" ...
```
...
# 55, not terrible, spot check with google finds that many of these are not protein-coding
...
expr_duke_cpm <- cpm(expr_duke_protcoding, log = FALSE)
str(expr_duke_cpm)
...
```{r}
# will use "soft cutoff" based on library size:
...
# check results
dim(expr_duke_cpm)
expr_duke_cpm_filtered = expr_duke_cpm[keep,]
...
sample_pheno <- pheno_data_joined %>% pull(sample_id)
...
pheno_data_df <- as.data.frame(pheno_data_joined) %>%
...
annotation <- as.character("RNAseq counts derived from raw sequence data published by the Dave lab at Duke in Reddy et al Cell 2017. Quantified with Kallisto with reverse strand parameter, collected with tximport, converted to CPM, filtered out genes expressed less than 1 in half or more of cases, TMM normalized and log2-transformed.")
...
```{r}
saveRDS(duke_es, "output/duke_expression_set_cleaned_log.rds")
