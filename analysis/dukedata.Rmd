...
gencode_gtf_split <- gencode_gtf %>% 
  ...
  dplyr::select(gene_id_v22 = gene_id, gene_name, geneID_split, everything()) %>%
  print()
...
# same but for hugo gene names
expr_duke_hgnc <- expr_duke %>%
  as.data.frame() %>%
  rownames_to_column(var = "gene_id_v29") %>%
  as_tibble() %>%
  mutate(geneID_split = str_replace(gene_id_v29, pattern = "\\.*", "")) %>%
  ...
  dplyr::select(-gene_status, -gene_id_v29, -gene_id_v22, -level, -havana_gene) %>%
  print()

# check how many gene ids are missing from version 22  
summary(is.na(expr_duke_hgnc$gene_name))
...
expr_duke_hgnc_filtered <- expr_duke_hgnc %>% 
  dplyr::filter(gene_type == "protein_coding")
...
expr_duke_hgnc_protcoding <- expr_duke_hgnc_filtered %>%
  dplyr::select(-geneID_split, -gene_type, -gene_name) %>%
  as.matrix()
rownames(expr_duke_hgnc_protcoding) <- expr_duke_hgnc_filtered$gene_name
expr_duke_hgnc_protcoding_ave <- avereps(expr_duke_hgnc_protcoding)
str(expr_duke_hgnc_protcoding_ave)
expr_duke_hgnc_protcoding_ave[1:5, 1:5]
```
...
ncol(expr_duke_hgnc_protcoding_ave)
expr_duke_hgnc_subset <- expr_duke_hgnc_protcoding_ave[, colSums(expr_duke_hgnc_protcoding_ave != 0) > gene_coverage_threshold]
expr_duke_hgnc_subset[1:5, 1:5]
ncol(expr_duke_hgnc_subset)
```
...
expr_duke_hgnc_cpm <- cpm(expr_duke_hgnc_subset, log = FALSE)
str(expr_duke_hgnc_cpm)
expr_duke_hgnc_cpm[1:5, 1:5]

```

Write out mixture file for deconvolution
```{r}
# TMM normalization
hgnc_norm_factors <- calcNormFactors(expr_duke_hgnc_cpm, method = "TMM")
summary(hgnc_norm_factors)

# convert expression matrix to dataframe
expr_duke_hgnc_cpm_df <- as.data.frame(expr_duke_hgnc_cpm)

# apply factor to each column
total_cpm_norm_duke <- map2_dfc(expr_duke_hgnc_cpm_df, hgnc_norm_factors, `*`)
total_cpm_norm_duke <- as.data.frame(expr_duke_hgnc_cpm_df)
total_cpm_norm_duke[1:5, 1:5]
str(total_cpm_norm_duke)
duke_dlbcl_mixture <- total_cpm_norm_duke %>%
  rownames_to_column(var = "GeneSymbol") %>%
  as_tibble() %>%
  print()

write_delim(duke_dlbcl_mixture, 
            "output/duke_dlbcl_mixture.tsv",
            delim = "\t",
            col_names = TRUE)
