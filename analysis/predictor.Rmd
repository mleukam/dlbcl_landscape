...
```{r}
# For deconvolution
# Get Hugo gene name expression matrix in non-log space
uchi_expr_hugo <- uchi_expr_df %>% 
  dplyr::select(-geneID_split)

uchi_hugo_matrix <- uchi_expr_hugo[-1] %>% as.matrix()
rownames(uchi_hugo_matrix) <- uchi_expr_df$gene_name
dim(uchi_hugo_matrix)
uchi_hugo_matrix_ave <- avereps(uchi_hugo_matrix)
dim(uchi_hugo_matrix_ave)

uchi_mixture <- exp(uchi_hugo_matrix_ave) %>% 
  as.data.frame() %>%
  rownames_to_column(var = "GeneSymbol")
uchi_mixture[1:5, 1:5]

write_delim(uchi_mixture, "output/dlbcl_uchi_batch1_mixture.tsv", delim = "\t")
```
...
Read in PCA models from GSVA v4 and v5 gene sets on combined DLBCL RNAseq dataset (Duke + NCI, n = 1189, 143 gene sets). Clustered into 4 groups using HCPC. These are all from the [GSVA v4 score analysis notebook](visualizing_gsva_scores_4.html) and [GSVA v5 score analysis notebook](visualizing_gsva_scores_5.html).
...
dlbcl_pca <- readRDS("output/dlbcl_gsva4_5cluster_pca.rds")
dlbcl_hcpc <- readRDS("output/dlbcl_gsva4_5cluster_hcpc.rds")
...
box_4v1 <- ggplot(data = uchi_pheno_1v4,
  ...
  scale_fill_manual(values = mypal) +
  theme_classic2() +
  labs(title = "Inflamed vs Cold cluster IHC") +
  ylab("IHC Score") +
  xlab("") +
  theme(legend.position = "none") +
