...
l_cut <- (0.23247 + 0.178593 + 0.1258525 + 0.126851) / 4
...
ego2 <- enrichGO(gene = go_gene_list[[3]],
                 OrgDb = org.Hs.eg.db,
                 ont = "BP",
                 pvalueCutoff = 0.05,
                 pAdjustMethod = "BH",
...
# convert to gene names
ego2 <- setReadable(ego1, OrgDb = org.Hs.eg.db)
...
# plots
pathways1 <- hallmark_results[[1]] %>% arrange(desc(abs(NES))) %>% pull(pathway)
ranks = ranklist[[1]]
