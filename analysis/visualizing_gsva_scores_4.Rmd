...
coo_sum_prop <- coo_sum %>%
  dplyr::rename(coo = abc_gcb_rna) %>%
  left_join(clust1_prop, by = c("coo", "clust")) %>%
  dplyr::select(-cold, -not_cold) %>%
  left_join(clust2_prop, by = c("coo", "clust")) %>%
  dplyr::select(-intermediate_t, -not_intermediate_t) %>%
  left_join(clust3_prop, by = c("coo", "clust")) %>%
  dplyr::select(-intermediate_m, -not_intermediate_m) %>%
  left_join(clust4_prop, by = c("coo", "clust")) %>%
  dplyr::select(-inflamed, -not_inflamed) %>%
  mutate(proportion = coalesce(proportion.x, 
                               proportion.y, 
                               proportion.x.x, 
                               proportion.y.y)) %>%
...
# make data frae containing all p-values
...
coo_bar <- ggplot(coo_sum_prop, 
  ...
  labs(title = "Cell of origin by cluster",
       ...
       x = "",
       y = "Number of cases") +
  geom_text(aes(label = percent), vjust = 2, color = "white") +
...
plot1 <- ggsurvplot(fit1, 
                    ...
                    tables.height = 0.3,
                    ggtheme = theme_classic2(),
                    title = "Overall survival by immune response cluster",
