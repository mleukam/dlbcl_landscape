...
## DGE by cluster
Recreate expression set for normalized, filtered counts
```{r}
...
annotation <- as.character("Duke RNAseq counts derived from raw sequence data published by the Dave lab at Duke in Reddy et al Cell 2017. Quantified with Kallisto with reverse strand parameter, collected with tximport, filtered for only protein-coding genes, selected for cases with expression in >12,000 genes, filtered out genes with counts < 10 in half or more of cases, TMM normalized. Not CPM normalized or log transformed. Batch corrected using ComBat by sequencing lane. NCI RNAseq counts originally published by the Staudt lab at NCI in Schmitz et al NEJM 2018. Raw counts downloaded from GDC as HTseq counts filtered out genes with counts < 10 in half or more of cases, TMM normalized. Not CPM normalized or log-transformed. Batch corrected for NCI vs Duke source using ComBat")

# assemble expressionset
counts_es <- ExpressionSet(
...
expr_matrix <- exprs(counts_es)

# To preserve linear relationships, will add a constant to every value
expr_adj <- expr_matrix + 5000
min(expr_adj)

# create DGEList
dge <- DGEList(counts = expr_adj)

# set up design matrix
rnames <- colnames(expr_adj)
groups <- pData(counts_es) %>% 
  pull(clust) %>%
  as.factor()
...
contr_matrix_c1 <- makeContrasts("clust_1_vs_others" = cluster_1 - (cluster_2 + cluster_3 + cluster_4) / 3, 
...
```{r}
# set up contrast matrix
contr_matrix_c2 <- makeContrasts("clust_1_vs_others" = cluster_2 - (cluster_1 + cluster_3 + cluster_4) / 3, 
  levels = colnames(design))
contr_matrix_c2
...
```{r}
# set up contrast matrix
contr_matrix_c3 <- makeContrasts("clust_1_vs_others" = cluster_3 - (cluster_1 + cluster_2 + cluster_4) / 3, 
  levels = colnames(design))
contr_matrix_c3
...
```{r}
# set up contrast matrix
contr_matrix_c4 <- makeContrasts("clust_1_vs_others" = cluster_4 - (cluster_1 + cluster_2 + cluster_3) / 3, 
  levels = colnames(design))
contr_matrix_c4
