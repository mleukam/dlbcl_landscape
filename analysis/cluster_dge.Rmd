...
## GSVA model version 5 - NCI only

```{r}
nci_clust_data <- readRDS("output/nci_pheno_gsva_clust_v5.rds")
```
...
```{r}
# create DGEList
dge <- DGEList(counts = total_nci_filtered)
...
identical(colnames(total_nci_filtered), rownames(nci_clust_data))
rnames <- rownames(nci_clust_data)
groups <- nci_clust_data %>% 
  pull(gsva_clust) %>%
  paste0("cluster_", .) %>%
  as.factor()
...
saveRDS(dge, "output/v5_nci_dgelist_for_limma.rds")
saveRDS(design, "output/v5_nci_design_for_limma.rds")
```
...
# write out results
saveRDS <- saveRDS(v, "/gpfs/data/kline-lab/tcga_macs/output/nci_limma_voom_elist_1.rds")

saveRDS <- saveRDS(fit, "/gpfs/data/kline-lab/tcga_macs/output/nci_limma_voom_marraylmfit.rds")
```

Read in results
```{r}
v <- readRDS("output/nci_limma_voom_elist_1.rds")
fit <- readRDS("output/nci_limma_voom_marraylmfit.rds")
```
...
nci_cluster1_dge <- topTable(fit_c1, 
                         adjust.method = "BH", 
                         sort.by = "none", 
                         number = Inf)

top50_cluster1_dge_up <- nci_cluster1_dge %>%
  rownames_to_column(var = "gene_id") %>%
...
top50_cluster1_dge_down <- nci_cluster1_dge %>%
  rownames_to_column(var = "gene_id") %>%
  left_join(gencode_gtf) %>%
  arrange(logFC) %>%
  dplyr::slice(1:50) %>%
...
nci_cluster2_dge <- topTable(fit_c2, 
                         adjust.method = "BH", 
                         sort.by = "none", 
                         number = Inf)

top50_cluster2_dge_up <- nci_cluster2_dge %>%
  rownames_to_column(var = "gene_id") %>%
  left_join(gencode_gtf) %>%
  arrange(desc(logFC)) %>%
  dplyr::slice(1:50) %>%
...
top50_cluster2_dge_down <- nci_cluster2_dge %>%
  rownames_to_column(var = "gene_id") %>%
  left_join(gencode_gtf) %>%
  arrange(logFC) %>%
  dplyr::slice(1:50) %>%
...
nci_cluster3_dge <- topTable(fit_c3, 
                         adjust.method = "BH", 
                         sort.by = "none", 
                         number = Inf)

top50_cluster3_dge_up <- nci_cluster3_dge %>%
  rownames_to_column(var = "gene_id") %>%
  left_join(gencode_gtf) %>%
  arrange(desc(logFC)) %>%
  dplyr::slice(1:50) %>%
...
top50_cluster3_dge_down <- nci_cluster3_dge %>%
  rownames_to_column(var = "gene_id") %>%
  left_join(gencode_gtf) %>%
  arrange(logFC) %>%
  dplyr::slice(1:50) %>%
...
nci_cluster4_dge <- topTable(fit_c4, 
                         adjust.method = "BH", 
                         sort.by = "none", 
                         number = Inf)

top50_cluster4_dge_up <- nci_cluster4_dge %>%
  rownames_to_column(var = "gene_id") %>%
  left_join(gencode_gtf) %>%
  arrange(desc(logFC)) %>%
  dplyr::slice(1:50) %>%
...
top50_cluster4_dge_down <- nci_cluster4_dge %>%
  rownames_to_column(var = "gene_id") %>%
  left_join(gencode_gtf) %>%
  arrange(logFC) %>%
  dplyr::slice(1:50) %>%
...
cluster4_dge_names <- nci_cluster4_dge %>%
  as.data.frame() %>%
  rownames_to_column(var = "gene_id") %>%
  as_tibble() %>%
  left_join(gencode_gtf, by = "gene_id") %>%
...
```{r}
saveRDS(nci_cluster1_dge, "output/nci_cluster1_dge.rds")
saveRDS(nci_cluster2_dge, "output/nci_cluster2_dge.rds")
saveRDS(nci_cluster3_dge, "output/nci_cluster3_dge.rds")
saveRDS(nci_cluster4_dge, "output/nci_cluster4_dge.rds")
```
...
saveRDS(pheno_data, "output/dgelist_pheno_data.rds")
